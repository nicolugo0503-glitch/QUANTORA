// Quantora backend - Global Events. Cached GDELT proxy. Uses simple single-term queries
// (GDELT rejects complex OR/quoted queries) merged server-side for broad coverage.
module.exports = async (req, res) => {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Cache-Control', 's-maxage=900, stale-while-revalidate=86400');
  const TERMS = ['markets', 'economy', 'inflation', 'bitcoin', 'recession'];
  ...
  try {
    ...
    for (let i = 0; i < TERMS.length; i++) {
      const j = await gj(art + encodeURIComponent(TERMS[i]));
      if (j && Array.isArray(j.articles)) all = all.concat(j.articles);
    }
    const seen = {}, uniq = [];
    ...
    const t = await gj('https://api.gdeltproject.org/api/v2/doc/doc?mode=timelinetone&format=json&timespan=7d&query=' + encodeURIComponent('markets'));
    const tone = (t && t.timeline && t.timeline[0] && t.timeline[0].data) || [];
