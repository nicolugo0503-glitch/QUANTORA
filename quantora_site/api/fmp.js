...
async function marketDataTool(input){
  ...
  for(var i=0;i<fields.length;i++){ var fld=fields[i];
    if(fld==='quote'){ var q=await f('quote?symbol='+sym); q=Array.isArray(q)?q[0]:q; if(!(q&&q.price!=null)){ try{ var _pq=await fetch('https://www.usequantora.com/api/fmp?type=prices&symbol='+encodeURIComponent(sym)).then(function(r){return r.json();}); var _pa=Array.isArray(_pq)?_pq:((_pq&&_pq.data)||[]); if(_pa[0]) q={ price:(_pa[0].price!=null?_pa[0].price:_pa[0].close) }; }catch(_e){} } if(q) out.quote={ price:q.price, changePct:(q.changePercentage!=null?q.changePercentage:q.changesPercentage), marketCap:q.marketCap, pe:q.pe, name:q.name, dayHigh:q.dayHigh, dayLow:q.dayLow, yearHigh:q.yearHigh, yearLow:q.yearLow }; }
    else if(fld==='prices'||fld==='returns'){ var pr=await fetch('https://www.usequantora.com/api/fmp?type=prices&symbol='+encodeURIComponent(sym)).then(function(r){return r.json();}).catch(function(){return null;}); var arr=Array.isArray(pr)?pr:((pr&&pr.data)||[]); var closes=arr.slice(0,260).map(function(x){return x.price!=null?x.price:x.close;}).filter(function(x){return x!=null&&isFinite(x);}); var rets=[]; for(var k=0;k<closes.length-1;k++){ rets.push((closes[k]-closes[k+1])/closes[k+1]); } out.returns=rets.slice(0,250).reverse(); out.lastClose=closes[0]; }
